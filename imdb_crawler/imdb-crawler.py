...
def scrape_top_250(soup):
    ...
    for film in soup.find_all('td', class_ = 'titleColumn'):
        for name in film.find_all('a'):
            url = name.get('href')
...
def scrape_movie_page(dom):
    ...
    for film in dom.find_all('div', class_ = 'title_block'):
        
        # Scrapes the title of the movie
        title = film.h1.text
        #title = dom.find('div', class_ = 'title_wrapper').h1.text

        # Scrapes the year of the movie
        year = film.h1.span.text
        #year = dom.find(id='titleYear').a.text

        # Scrapes the genre(s) of the movie
        genre_list = []
        for genre in film.select('a[href*="genres=action"]'):
    ...
    # Scrapes the duration of the movie
    duration = dom.find('time', datetime_ = 'PT').text
    duration = int(duration.split()[0])



    # Scrapes the director(s) of the movie
    director_list = []
    for director in dom.find('a[href*="name"]'):
        director_list.append(director.text)

    director = '; '.join(director_list)

    # Scrapes the writer(s) of the movie
    writer_list = []
    for writer in dom.select('a[href*="name"]'):
        writer_list.append(writer.text)

    writer = '; '.join(writer_list)

    # Scrapes the actors of the movie
    actor_list = []
    for actor in dom.select('a[href*="name"]'):
        actor_list.append(actor.text)

    actors = '; '.join(actor_list)

    # Scrapes the rating of the movie
    rating = float(dom.find(itemprop = 'ratingValue').text)

    # Scrapes the amount of ratings of the movie
    number_ratings = dom.find(itemprop = 'ratingCount').text

    
    # Appends all the acquired data to a CSV    
    movie.append(title)
    #movie.append(year)
    ...
    movie.append(actors)
    movie.append(rating)
    movie.append(number_ratings)

    print(movie)

    #movie.extend(year, duration, genre, director, writer, actors, rating, number_ratings)
